...
def get_titan_response(model, input_content, temperature, top_p, max_token_count, stop_pipe_count): #text-to-text client function
    ...
    model_kwargs = { #For the LangChain Bedrock implementation, these parameters will be added to the textGenerationConfig item that LangChain creates for us
        ...
        "stopSequences": [('|' * stop_pipe_count)], #Titan just uses pipes for stop sequences
